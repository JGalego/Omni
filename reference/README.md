...
CI runs all five over a corpus this repository can build — two models, two pairs,
two templates, one tokenizer, one family — which measures the instrument rather
than the gate, and exits 3 saying so. The family case is checked in both
directions, because a comparison that cannot fail is not a comparison: it is run
once against this build's own answers and once against a deliberately corrupted
expectation.
