...
[`tools/corpus.py`](../tools/corpus.py) is the instrument for all four:
...
CI runs all four over a corpus this repository can build — two models, two pairs,
two templates, one tokenizer — which measures the instrument rather than the
gate, and exits 3 saying so.
