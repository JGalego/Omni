...
- **Zero dependencies.** `docs/design/sdk.md` §5 claims a conforming C0 reader
  ...
  parameter sets, SLH-DSA in all twelve of FIPS 205's parameter sets, ChaCha20, deflate, Zstandard, LZ4, LZMA2 with its range coder,
...
- §12.5 signatures: COSE_Sign1 over the §12.5.2 payload, Ed25519 **and ES256**,
  ...
  well, in all twelve parameter sets — the six SHAKE and the six SHA2 — which
  completes §12.5.1: it rests on
  ...
  wrappers, domain-separated by an address whose every field is hashed, and
  an implementation that sets one field wrong or concatenates a node's children in
  the wrong order signs and verifies its own signatures perfectly. The SHA2 sets
  are the same scheme over a 22-byte compressed address, SHA-256 and SHA-512, and
  MGF1 and HMAC — a second construction wearing the same six names, and each of
  its twelve sets matches NIST byte for byte.
  What is *not* implemented is ML-DSA's pre-hash and
  external-mu interfaces — refused by name. COSE has assigned no identifiers
  for SLH-DSA, so the two sets this build offers as container signatures come
  from the private-use range and
...
Gate 1 and Gate 2 each ask for two numbers this repository cannot produce on its
own, and the reason is a corpus rather than a gap in the code.
[`tools/corpus.py`](../tools/corpus.py) is the instrument for all of them:

```console
$ tools/corpus.py roundtrip  ./models    --omni ./omni   # Gate 1: bit-exact
$ tools/corpus.py delta      ./pairs.txt --omni ./omni   # Gate 1: delta sizes
$ tools/corpus.py families   ./cases     --omni ./omni   # Gate 2: vs a framework
$ tools/corpus.py tokenizers ./repos     --omni ./omni   # Gate 2: vectors
$ tools/corpus.py templates  ./templates --omni ./omni   # Gate 2: OMNI-CT
$ omni jinja --coverage ./templates                      # the same, from the CLI
```

`families` is the one that needed a CLI change to be possible at all: comparing
a graph's outputs with a framework's needs the whole tensor, and `graph run` only
printed six values, so it gained `--json`. A case is a container, the tokens to
run it over, and what PyTorch computed in the same shape — each with its own
declared tolerance, because a bf16 model and an f32 one do not deserve the same
one.

Each prints the gate's own threshold beside its figure and **exits 3 when the
corpus is short of it**, so pointing one at a hundred real models is a check and
not a report. Two of them report something apart from their headline number on
purpose: `roundtrip` counts a *refusal with a reason* separately from a failure,
because a format feature this build declines to guess at is a different outcome
from a model that did not survive; and `tokenizers` counts how many tokenizers
ran conformance vectors separately from how many imported, because vectors have
to come from the source tokenizer and one generated by the encoder it is meant to
test cannot fail.

CI runs all five over a corpus this repository can build, and two of the five are
no longer measuring only the instrument.

The **tokenizer** vectors now come from Hugging Face `tokenizers`
([`tools/tokenizer-vectors.py`](../tools/tokenizer-vectors.py)), so
`verify --tokenizer` is comparing this build's encoder against one it shares no
code with. That number had been zero since the harness was written, for a reason
that was sound and had gone unexamined: vectors have to come from the source
tokenizer, so none were available, so nothing was checked. The first run against
a real library found that `Whitespace` was implemented as `ByteLevel`'s
keep-the-space behaviour and put `<unk>` between every pair of words.

The **family** comparison now runs against PyTorch
([`tools/torch-reference.py`](../tools/torch-reference.py)), which implements
§07.5's decoder from the graph `omni graph` prints. It agrees to about 0.3 % of
the largest logit, which is what bf16 with a different accumulation order costs,
and the comparison is checked for discriminating power rather than assumed to
have it: a wrong GQA head mapping shows up at 120 % of signal, the wrong RoPE
pairing at 4 %, and omitting RoPE at 3 %. What used to be here compared this
build's answers with themselves, over `omni example --graph`, whose weights are
pseudo-random *bytes* — as bf16 they are NaNs and values near 1e24, and the graph
over them evaluates to all zeros. It compared 768 zeros with 768 zeros.

Both are still short of what the gates ask, and both still exit 3 saying so: one
tokenizer against 200, one family against ten. The remaining three measure the
instrument and say that too. A corpus of published models is what is missing, and
on a machine that can reach a model hub these five commands are what to point at
it.

## Tests

670 tests covering: SHA-256 against FIPS 180-4 vectors; BLAKE3 against the
...
```console
$ cargo test
test result: ok. 670 passed; 0 failed
