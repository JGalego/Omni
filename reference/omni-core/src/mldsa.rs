...
//! against itself — see `tools/mldsa-vectors.py` for how the fixtures are
...
// ---------------------------------------------------------------------------
// Keccak-f[1600] and the two SHAKE functions FIPS 204 calls H and G.
// ---------------------------------------------------------------------------
...
impl Xof {
    fn new(rate: usize, parts: &[&[u8]]) -> Xof {
        let mut st = [0u64; 25];
        let mut i = 0usize;
        for part in parts {
            for &b in *part {
    ...
    fn shake128(parts: &[&[u8]]) -> Xof {
        Xof::new(168, parts)
    }

    fn shake256(parts: &[&[u8]]) -> Xof {
        Xof::new(136, parts)
    }
...
mod tests {
    ...
    fn shake_squeezes_across_a_rate_boundary() {
        ...
        let mut a = Xof::shake128(&[b"omni"]);
        let whole = a.squeeze_vec(400);
        let mut b = Xof::shake128(&[b"omni"]);
        let mut pieces = Vec::new();
        for n in [1usize, 7, 160, 32, 200] {
            pieces.extend_from_slice(&b.squeeze_vec(n));
        }
